...
class ErrorHandler:
    def __init__(self):
        ...
        try:
            ...
            logger.warning(f"Error handler initialized without WhatsApp sender: {e}")
    ...
    def send_error_whatsapp(self, phone_number: str, error_type: str, 
        ...
        try:
            # Check if WhatsApp sender is available
            if self.whatsapp_sender is None:
                logger.error("WhatsApp sender not initialized")
                return {
                    "messageSent": False,
                    "error": "WhatsApp sender not available",
